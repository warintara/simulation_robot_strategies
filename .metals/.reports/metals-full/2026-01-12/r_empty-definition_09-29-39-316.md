error id: file://<HOME>/T%C3%A9l%C3%A9chargements/simovies20251201/src/algorithms/TeamAMainBotXXX.java:algorithms/TeamAMainBotXXX#moveBack#
file://<HOME>/T%C3%A9l%C3%A9chargements/simovies20251201/src/algorithms/TeamAMainBotXXX.java
empty definition using pc, found symbol in pc: algorithms/TeamAMainBotXXX#moveBack#
...
public class TeamAMainBotXXX extends Brain {
    ...
    private void orientByTeam() {
        ...
        // Team A → orientation EST
        if (Math.abs(getHeading() - Parameters.EAST) > Parameters.teamAMainBotStepTurnAngle) {
            stepTurn(Parameters.Direction.RIGHT);
            sendLogMessage("[ORIENT] Turning EAST");
        } else {
            sendLogMessage("[ORIENT] Facing EAST");
        }
    }
...
private boolean detectEnemy() {
    ...
    private void joinCircle() {
        ...
        if (Math.abs(angleDiff) > Parameters.teamAMainBotStepTurnAngle) {
            if (angleDiff > 0) {
                stepTurn(Parameters.Direction.LEFT);
                sendLogMessage("[CIRCLE] Turning LEFT toward enemy");
            } else {
                stepTurn(Parameters.Direction.RIGHT);
                sendLogMessage("[CIRCLE] Turning RIGHT toward enemy");
            }
            return;
        }

        // Avancer prudemment vers la cible
        move();
    ...
    return;
        ...
        if (distance > 900) {
            move();
            sendLogMessage("[CIRCLE] Too far, closing in");
            return;
        }

        // Distance correcte → rotation circulaire
        stepTurn(Parameters.Direction.LEFT);
        move();
        sendLogMessage("[CIRCLE] Circling enemy");
    }
