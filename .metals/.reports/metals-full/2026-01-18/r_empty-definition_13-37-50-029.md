error id: file://<WORKSPACE>/src/algorithms/TeamAMainBotXXX.java:algorithms/TeamAMainBotXXX#stepTurn#
file://<WORKSPACE>/src/algorithms/TeamAMainBotXXX.java
empty definition using pc, found symbol in pc: algorithms/TeamAMainBotXXX#stepTurn#
...
public class TeamAMainBotXXX extends Brain {
    ...
    private static final int REPORT_DELAY = 15;
    ...
    private boolean enemyKnown = false;
    private double targetEnemyX;
    private double targetEnemyY;

    // Odométrie simple (approx)
    private double myX = Parameters.teamAMainBot1InitX;
    private double myY = Parameters.teamAMainBot1InitY;
    ...
    public void activate() {
        state = State.INIT;
        System.out.println("[MAIN] Activated (role not assigned yet)");
    }
    ...
    public void step() {
                ...
                // Priorité absolue : finir un virage
        if (turnTask) {
            if (Math.abs(Math.sin(getHeading() - targetHeading))
                < Parameters.teamAMainBotStepTurnAngle) {
                turnTask = false;
            } else {
                st@@epTurn(Parameters.Direction.RIGHT);
            }
            return;
        }
        if (reportCooldown > 0) {
        ...
        switch (state) {
            ...
            case FIRE:
                if (safeToFire()) {
                    ...
                    log("Fire blocked (friendly fire risk)");
                }
...
private void log(String msg) {
    if (DEBUG) {
        System.out.println("[MAIN-" + role + "] " + msg);
        sendLogMessage("[MAIN-" + role + "] " + msg);
    }
}
...
private void orientByTeam() {
    ...
    double diff = Math.sin(Parameters.EAST - getHeading());

    if (Math.abs(diff) > Parameters.teamAMainBotStepTurnAngle) {
...
private boolean orientationDone() {
    return Math.abs(Math.sin(getHeading() - Parameters.EAST)) 
           < Parameters.teamAMainBotStepTurnAngle;
}
...
private boolean detectEnemy() {
    for (IRadarResult r : detectRadar()) {
        if (r.getObjectType() == IRadarResult.Types.OpponentMainBot ||
            ...
            log("Enemy detected locally, updating target position");
            // position ABSOLUE
            targetEnemyX = myX + r.getObjectDistance() * Math.cos(r.getObjectDirection());
            targetEnemyY = myY + r.getObjectDistance() * Math.sin(r.getObjectDirection());
            enemyKnown = true;
...
private void broadcastEnemy() {
    ...
    broadcast("ENEMY:" + (int) enemyX + ":" + (int) enemyY);
    log("Enemy reported at (" + (int) enemyX + "," + (int) enemyY + ")");

    reportCooldown = REPORT_DELAY;
}
...
private void joinCircle() {
    ...
    myMove();
    log("Going to enemy, dist=" +
    (int) Math.hypot(targetEnemyX - myX, targetEnemyY - myY));
...
private void circleEnemy() {
    ...
    private boolean inCirclePosition() {
        double dist = Math.hypot(targetEnemyX - myX, targetEnemyY - myY);
        return dist > 600 && dist < 900;
    }
...
private boolean safeToFire() {
    ...
    double fireAngle = Math.atan2(
        targetEnemyY - myY,
        targetEnemyX - myX
    );

    double enemyDist = Math.hypot(
        targetEnemyX - myX,
        targetEnemyY - myY
    );
    ...
    for (IRadarResult r : detectRadar()) {
        ...
        if (r.getObjectType() == IRadarResult.Types.TeamMainBot ||
            ...
            double allyAngle = r.getObjectDirection();
            double allyDist = r.getObjectDistance();

            // allié entre moi et l'ennemi
            if (allyDist < enemyDist &&
                Math.abs(Math.sin(allyAngle - fireAngle)) < 0.15) {
                return false;
            }
        }
    }
    ...
    private void handleMessages() {
        ...
        for (String msg : messages) {
            ...
            if (msg.startsWith("ENEMY:")) {
                ...
                if (parts.length >= 3) {
                    targetEnemyX = Double.parseDouble(parts[1]);
                    targetEnemyY = Double.parseDouble(parts[2]);
                    enemyKnown = true;
                
                    log("Received enemy position (" +
                        (int) targetEnemyX + ", " + (int) targetEnemyY + ")");
...
private void goToEnemy() {
    ...
    double dx = targetEnemyX - myX;
    double dy = targetEnemyY - myY;
